#!/usr/bin/env bash
#
# VEDHA installer — one command, everything required.
#
...
#   7. Runs `vedha doctor` to confirm green
#
# Properties:
...
VEDHA_VERSION="0.6.0"
MIN_NODE_MAJOR=20
INSTALL_LOG="${TMPDIR:-/tmp}/vedha-install-$$.log"
DRY_RUN=false
VERBOSE=false
...
for arg in "$@"; do
  case $arg in
    ...
    --help|-h)
      cat <<EOF
VEDHA installer v${VEDHA_VERSION}
...
case "$(uname -s)" in
  ...
  Linux)  OS_KIND="linux" ;;
  *)      die "Unsupported OS: $(uname -s)" "VEDHA currently supports macOS and Linux. Windows: use WSL2." ;;
esac
...
step "Locating VEDHA repo"

if [[ -f "package.json" ]] && grep -q '"name": "vedha"' package.json 2>/dev/null; then
  REPO_DIR="$PWD"
  ...
  if [[ -d "$REPO_DIR" ]]; then
    ...
    run git clone --depth 1 https://github.com/your-org/vedha.git "$REPO_DIR"
...
# ── Bundled scanner tools (managed by VEDHA itself) ───────────────
# This is what makes client deployments hands-off: naabu, nuclei, httpx,
# subfinder, ffuf get downloaded by `vedha tools install` into
# ~/.vedha/tools/ with pinned versions. No go install, no homebrew.
...
if $DRY_RUN; then
  ...
  npx tsx cli/index.ts tools install \
    && ok "Bundled tools installed into ~/.vedha/tools/" \
...
if [[ "$OS_KIND" == "macos" ]] && ! brew list seclists >/dev/null 2>&1; then
  ...
  brew install --quiet seclists >> "$INSTALL_LOG" 2>&1 || warn "seclists install failed — ffuf will need VEDHA_FFUF_WORDLIST set"
fi
...
if [[ -f .env.local ]]; then
  ...
  if $DRY_RUN; then
    ...
    cat > .env.local <<EOF
# VEDHA — auto-generated by install.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ)
...
RESEND_API_KEY=
RESEND_FROM=Vedha <noreply@vedha.security>
...
# ── Final verification — run `vedha doctor` ──────────────────────
