/**
 * Native TCP port scanner — no external binaries required.
 *
 * Uses Node's net.Socket to do a TCP connect scan, concurrently across the
 * supplied port list. Banner-grabs on connect when a service speaks first
 * (FTP, SSH, SMTP, MySQL, Redis, etc.).
 *
 * Trade-offs vs naabu:
 *   + Zero install, runs on every platform out of the box
 *   + Banner grabs come for free
 *   - Slower than naabu (naabu can do 10k pps; this peaks ~500 connections)
 *   - TCP connect is louder than naabu's -s c (full handshake) — same actually
 *
 * For most casual / lab work this is more than enough. Power users can install
 * naabu and the dispatcher will prefer it.
 */
import { Socket } from 'net';
import type { DiscoveredHost } from '../types';

// Top 1000 TCP ports per nmap-services (popularity-ranked, deduped).
// This is the same list nmap uses with no -p flag. Embedded so we never need
// an external services file.
const TOP_1000_TCP: number[] = [
...
// Service name lookup for the most common ports (no system services file dep).
...
export type PortRange = 'top-100' | 'top-1000' | 'all' | number[];

function resolvePorts(range: PortRange): number[] {
  if (Array.isArray(range)) return range;
  if (range === 'top-100')  return TOP_1000_TCP.slice(0, 100);
  if (range === 'top-1000') return TOP_1000_TCP;
  if (range === 'all')      return Array.from({ length: 65535 }, (_, i) => i + 1);
  return TOP_1000_TCP;
}

/** Resolve target to a list of IPs (single IP, hostname, or /24 CIDR). */
function expandTarget(target: string): string[] {
  if (target.includes('/')) {
    const [base, prefixStr] = target.split('/');
    const prefix = parseInt(prefixStr, 10);
    if (prefix < 16 || prefix > 32) return [base]; // refuse huge ranges
    ...
    for (let i = network; i <= broadcast; i++) {
      out.push([(i >>> 24) & 255, (i >>> 16) & 255, (i >>> 8) & 255, i & 255].join('.'));
    }
    return out;
  }
  return [target];
...
/** Single host:port TCP connect probe. */
function checkPort(ip: string, port: number, opts: CheckOpts): Promise<NativePortResult | null> {
  return new Promise((resolve) => {
    const timeoutMs = opts.timeoutMs ?? 1500;
    const sock = new Socket();
    let banner = '';
    let resolved = false;
    const done = (result: NativePortResult | null): void => {
      if (resolved) return;
      resolved = true;
      sock.destroy();
      resolve(result);
    ...
    sock.on('connect', () => {
      if (!opts.grabBanner) {
        done({ ip, port, service: PORT_NAMES[port] });
        return;
      }
      // Hold the socket open briefly to collect banner from services that
      // greet on connect (SSH, FTP, SMTP, MySQL, Redis, etc.)
      const bannerTimeout = setTimeout(() => {
        done({ ip, port, banner: banner.slice(0, 200) || undefined, service: PORT_NAMES[port] });
      }, 600);
      sock.on('data', (chunk: Buffer) => {
        banner += chunk.toString('utf8');
        if (banner.length > 1000) {
          clearTimeout(bannerTimeout);
          done({ ip, port, banner: banner.slice(0, 200), service: PORT_NAMES[port] });
        }
      });
    });
...
export interface NativeScanOpts {
  ports?:       PortRange;
  concurrency?: number;
  timeoutMs?:   number;
  grabBanner?:  boolean;
...
/**
 * Scan targets across the given port range with controlled concurrency.
 * Returns a flat list of open ports; group later into DiscoveredHost objects.
 */
export async function nativePortScan(
  targets: string[],
  opts: NativeScanOpts = {},
): Promise<NativePortResult[]> {
  const ports = resolvePorts(opts.ports ?? 'top-1000');
  const ips   = targets.flatMap(expandTarget);
  const concurrency = opts.concurrency ?? 200;
  const timeoutMs   = opts.timeoutMs   ?? 1500;
  const grabBanner  = opts.grabBanner  ?? true;

  const queue: Array<[string, number]> = [];
  for (const ip of ips) for (const p of ports) queue.push([ip, p]);

  const out: NativePortResult[] = [];
  let active = 0; let next = 0; let done = 0;

  await new Promise<void>((resolve) => {
    const launch = (): void => {
      while (active < concurrency && next < queue.length) {
        const [ip, port] = queue[next++];
        active++;
        checkPort(ip, port, { timeoutMs, grabBanner }).then((r) => {
          active--; done++;
          if (r) out.push(r);
          if (opts.onProgress && done % 50 === 0) opts.onProgress(done, queue.length);
...
/** Group port-scan results into DiscoveredHost shape (compatible with naabu output). */
export function groupResults(results: NativePortResult[]): DiscoveredHost[] {
  ...
  for (const r of results) {
    if (!byIp.has(r.ip)) {
      byIp.set(r.ip, {
        ...
        ports:    [],
        services: [],
        discoveryMethod: 'tcp-syn-ping',
      });
    }
    const h = byIp.get(r.ip)!;
    if (!h.ports.includes(r.port)) {
      h.ports.push(r.port);
      h.services.push({
        port:  r.port,
        proto: 'tcp',
  ...
  for (const h of byIp.values()) h.ports.sort((a, b) => a - b);
  return [...byIp.values()];
}
