...
export interface BackendOpts {
  ...
  token?: string | null;
  body?: unknown;
  /** Pass through uploads or other non-JSON request bodies without re-encoding. */
...
/**
 * Low-level FastAPI transport. Returning the raw response lets BFF handlers
 * preserve files, status codes, and response headers without lossy JSON hops.
 */
