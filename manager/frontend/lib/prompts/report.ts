...
export interface ReportFinding {
  ...
  // ── Decomposed-prompt additions (report-sections.ts §5), all optional so the
  //    single-call schema stays valid ────────────────────────────────────────
  /** 1–2 sentences on what produced this severity (vector + exploit signals). */
  severity_rationale?: string;
  /** Set when severity looks inconsistent with the signals; a note for the reviewer. */
  severity_flag?: string | null;
  first_seen?: string | null;
  /** Advisory / CVE links surfaced in the finding's provenance strip. */
  references?: string[];
  // ── Exploit signals — scanner facts, not model output. Power the exposure
  //    matrix and the per-finding severity-rationale strip. ───────────────────
  cvss?: number | null;
...
export interface RemediationGroup {
  ...
  asset_count: number;
  verification: string;
  /** Fix IDs that must complete before this one (report-sections.ts §4, G8). */
  depends_on?: string[];
}

/** §1 board decision — a call only a decision-maker can make, derived from the
 *  remediation plan (report-sections.ts VERDICT_AND_SUMMARY_PROMPT). */
...
/** §6 controls-that-held entry, bounded to what was actually tested. */
...
export interface ReportResult {
  ...
  limitations: string[];
  /** Computed by buildScorecard, never generated by the model. */
  scorecard: Scorecard;
  // ── Decomposed-prompt additions, all optional so the flat single-call schema
  //    remains a valid ReportResult ─────────────────────────────────────────
  /** §1 board decisions derived from the remediation plan. */
  decisions?: ReportDecision[];
  /** §3 the single cheapest finding ref to break the attack chain. */
  cheapest_break?: string | null;
  /** §3 preconditions the attack chain requires. */
  preconditions?: string[];
  /** §4 stated reason the immediate window is empty, when nothing qualified. */
  immediate_window_empty_reason?: string | null;
  /** §6 richer controls detail (controls_observed keeps the flat string list). */
  controls?: ControlItem[];
  /** §7 method paragraph (what was tested, when, under what authorisation). */
  method?: string;
  /** How many findings were written up in full vs. tabulated (bounded per-finding gen). */
  findings_detailed?: number;
}

// ── Per-section output shapes (report-sections.ts). The orchestrator parses each
//    section into one of these before assembling a ReportResult. ──────────────
export interface VerdictSummaryOut { verdict: string; summary: string; decisions: ReportDecision[]; }
export interface AttackNarrativeOut {
  narrative: string | null; cheapest_break: string | null;
  preconditions: string[]; confidence_caveat: string | null;
}
export interface RemediationGroupingOut {
  groups: RemediationGroup[]; immediate_window_empty_reason: string | null;
}
export interface FindingWriteupOut {
  ref: string; title: string; business_impact: string; technical_detail: string;
  evidence_summary: string; severity_rationale: string; severity_flag: string | null;
  remediation_detail: string; verification: string; compliance_refs: string[];
}
export interface ControlsObservedOut { items: ControlItem[]; }
export interface LimitationsOut { method: string; limitations: string[]; }
