# Node Description Batch 65 of 227
...
- "agent_transport_enrollment_conflict_detail": "_enrollment_conflict_detail()" | kind=code-symbol | source=probe/agent/transport.py:L46 | neighbors=[transport.py, Best-effort extraction of the manager's…, .create_enrollment_request()]
- "agent_transport_transport_activate_enrollment": ".activate_enrollment()" | kind=code-symbol | source=probe/agent/transport.py:L351 | neighbors=[Transport, .load_state(), .update_state()]
- "agent_transport_transport_create_enrollment_request": ".create_enrollment_request()" | kind=code-symbol | source=probe/agent/transport.py:L332 | neighbors=[Transport, DeviceAlreadyEnrolledError, _enrollment_conflict_detail()]
