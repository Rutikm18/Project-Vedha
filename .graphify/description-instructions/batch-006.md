# Node Description Batch 7 of 336
...
- "agent_agent_run_ws_push_loop": "_run_ws_push_loop()" | kind=code-symbol | source=probe/agent/agent.py:L716 | neighbors=[agent.py, main(), Persistent WebSocket push loop.      Re…, _dbg(), _flush_spool_over_http(), say()]
...
- "agent_engine_run_scan": "run_scan()" | kind=code-symbol | source=probe/agent/engine.py:L570 | neighbors=[engine.py, Execute a scan and return the enriched …, _build_run_stats(), _derive_post_stage(), _error_result(), _facts_from_cache()]
