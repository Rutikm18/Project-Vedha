# Node Description Batch 62 of 186
...
- "probe_push_results_main": "main()" | kind=code-symbol | source=probe/push_results.py:L47 | neighbors=[push_results.py, die(), load_facts()]
...
- "routers_agents_agentbootstraprequest": "AgentBootstrapRequest" | kind=code-symbol | source=manager/backend/app/routers/agents.py:L565 | neighbors=[agents.py, BaseModel, .validate_network_segments()]
- "routers_agents_refresh_agent_registration": "refresh_agent_registration()" | kind=code-symbol | source=manager/backend/app/routers/agents.py:L841 | neighbors=[agents.py, _agent_ownership_check(), _scope_is_reachable()]
