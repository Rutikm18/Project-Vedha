...
## Corpus Check
- Large corpus: 620 files · ~501,786 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder, or use --no-semantic to run AST-only.

## Summary
- 5360 nodes · 11816 edges · 292 communities detected
- Extraction: 77% EXTRACTED · 23% INFERRED · 0% AMBIGUOUS · INFERRED: 2776 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output
- Edge kinds: uses: 2776 · contains: 2480 · calls: 1714 · method: 1305 · MODIFIES: 1210 · rationale_for: 1068 · imports: 450 · imports_from: 416 · inherits: 183 · ON_BRANCH: 157 · PARENT_OF: 57


## Input Scope
- Requested: auto
- Resolved: committed (source: default-auto)
- Included files: 620 · Candidates: 1441
- Excluded: 42 untracked · 63604 ignored · 5 sensitive · 6 missing committed
- Recommendation: Use --scope all or graphify.yaml inputs.corpus for a knowledge-base folder.

## Graph Freshness
- Built from Git commit: `75650c1`
...
Cohesion: 0.02
Nodes (88): hw_bind.py — hardware fingerprinting for binary host-locking.  The compiled bina, agent — the probe transport layer (sealed, push-driven, hardware-bound).  Archit, result_spool.py — local result persistence with upload retry.  When the probe co, bytes_to_pubkey_b64(), decrypt_scope(), decrypt_scope_b64(), encrypt_scope(), encrypt_scope_b64() (+80 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (48): BloodHoundCollector — wrapper around the BloodHound.py collector + a Neo4j inges, NTLMRelayChecker — detect missing SMB/LDAP signing that enables NTLM relay.  NTL, HallucinationGuard — post-generation validation of LLM report text against the g, 298a9d4 trim frontend to 7 core pages; add use-case library, scope re-validation, result persistence, 65f22a7 Add comprehensive tests for authentication and admin seeding- Implement tests for the authentication login flow covering various scenarios including login success, user not found, password mismatch, disabled user/tenant, expired password, bcrypt failure, database failure, and JWT failure.- Introduce tests for the seed_admin script to ensure proper tenant and admin creation, password rotation, environment validation, and drift detection.- Create a Docker Compose configuration for the production stack, including services for PostgreSQL, Redis, Neo4j, and the API.- Add a probe configuration file for managing engagement scope and secure transport settings., d1b4dd3 trim frontend to 7 core pages; add use-case library, scope re-validation, result persistence, RateLimiter — enforces PPS limits per CIDR and business-hour windows from the en, NucleiExploitRunner — CVE PoC validation using Nuclei templates.  Enforces templ (+40 more)
...
Cohesion: 0.10
Nodes (92): agent.py — AgentDecisionEngine: the agentic AI advisor.  WHAT IT IS: a Claude to, GraphBuilder — turns engagement assets/services/findings into an attack graph., Build the full multi-type attack graph. Returns the populated DiGraph         (a, For each exploitable finding add an EXPLOITS edge Finding→Asset with         ``w, Add CONNECTS_TO (directed reachability) and SAME_SEGMENT edges from         segm, CREDENTIAL_REUSE edges between assets sharing a credential.         ``credential, Load assets/services/findings for an engagement and build the graph., Mirror the current in-memory graph into Neo4j via batched writes. (+84 more)
...
Cohesion: 0.03
Nodes (23): Probe test suite — unit tests for the probe's pure-logic modules. Covers: ScopeG, _scan_result(), TestAssetMergeCredentialed, TestAssetMergeHostDiscovery, TestAssetMergePassiveCollect, TestAssetMergePortScan, TestAssetMergeServiceBanner, TestAssetMergeSmbScan (+15 more)
...
Cohesion: 0.06
Nodes (26): get_settings(), Settings, BaseSettings, 10ceaca feat: implement AI model fallback mechanism and enhance model selection UI- Added support for fallback models when the primary AI provider is out of credit.- Introduced ModelSwitcher component for selecting AI models with improved UI.- Updated AssistantDrawer to handle fallback responses and display relevant information.- Enhanced API response structures to include fallback status.- Modified login form to allow explicit credential passing for testing.- Improved global styles for model selection and fallback notifications., 75650c1 feat: add Posture & Patch-Comparison Scorecard design spec, AiGenerateRequest, AiGenerateResponse, AiMessage (+18 more)
...
Cohesion: 0.04
Nodes (44): ComplianceRef, COVERAGE_COLOR, DetectionCoverage, ExploitMaturity, Finding, FindingDetail(), FindingPage, FindingsPage() (+36 more)

### Community 12 - "Community 12"
Cohesion: 0.05
Nodes (45): 045c9ae fix(posture): normalize run_at in _present_in_run; drop dead scores_prev call; tidy test import, 0f0097b feat(posture): mirror posture scorecard into generated reports, 237a831 feat(posture): add run comparison, matrix, and response builder, 5238865 feat(posture): add pure scoring core (noisy-OR risk/exploit/posture), a0b870c fix(posture): score over open findings only; lock grade-band boundary test, fadb4f5 fix(posture): hoist report-section test import; flush before section count, approve_report(), _build_engagement_summary() (+37 more)
...
Cohesion: 0.05
Nodes (55): DeclarativeBase, agent_recommendation.py — decisions/actions proposed by the agentic AI advisor., Append-only ledger of every attack action performed during an engagement.      W, Immutable, append-only audit trail for all exploit actions.     No TimestampMixi, Base, TimestampMixin, UUIDMixin, Per-engagement SIEM + EDR connection settings used by the detection     validati (+47 more)

### Community 15 - "Community 15"
Cohesion: 0.12
Nodes (45): transport.py — all manager communication (HTTP + WebSocket) in one place.  Encap, use_cases.py — the finite, pre-defined library of scan scenarios the manager can, backup-before-secret-removal, feat/probe-usecase-alignment, main, spike/probe-go, worktree-fleet-already-downloaded-cmd, 01f4398 feat(probe): IoT survey reaches the banner stage (service_fingerprint) (+37 more)
...
Cohesion: 0.05
Nodes (29): Agent, AGENT_STATUS, AgentStatus, PATH_STATUS, SEV_LABEL, Engagement, Finding, FindingSummary (+21 more)

### Community 18 - "Community 18"
Cohesion: 0.07
Nodes (42): BaseModel, create_findings_from_facts(), detect_findings_from_facts(), _ensure_importable(), engine_bridge.py — run the deterministic detection_engine on a probe's RAW FACTS, New raw-facts path: detect CVE findings from result['facts'] and persist     the, Background entry point (P1: keep detection OFF the probe-result request     path, (content_hash, fetched_at) of the pinned snapshot the engine will use, so     ev (+34 more)
...
Cohesion: 0.04
Nodes (13): Tests for agent/transport.py, Create a Transport with a real state file path but no actual HTTP calls., Create a Transport with a real state file path but no actual HTTP calls., TestDeviceEnrollment, TestFetchScope, TestHeartbeat, TestHttpGet, TestIdentity (+5 more)

### Community 22 - "Community 22"
Cohesion: 0.09
Nodes (37): AgentUnavailableError, Raised when the Anthropic SDK or API key is not configured., Base, AgentRecommendation, AttackPath, AttackTimeline, DetectionConfig, DetectionResult (+29 more)
...
Cohesion: 0.07
Nodes (23): _detect_drift(), _hash(), _log(), log_error(), log_info(), log_warn(), main(), Warn if the tenant has multiple admins or a stale admin email. (+15 more)
...
Cohesion: 0.07
Nodes (27): _clean(), _Collector, Make a per-host scanner instance share ONE rate limiter + semaphore with all, Make a raw banner safe and readable for the summary line.      Many services ans, _rollup(), _run_active(), _shared(), _coverage() (+19 more)

### Community 31 - "Community 31"
Cohesion: 0.09
Nodes (33): Agent, AgentStatus, _agent_can_execute_job(), _agent_ownership_check(), AgentBootstrapRequest, AgentRegisterRequest, AgentRegisterResponse, bootstrap_agent() (+25 more)

### Community 32 - "Community 32"
Cohesion: 0.08
Nodes (20): AssetCriticality, UserRole, OrderedDict, Attack path analysis API (AttackPathService).  GET /engagements/{id}/attack-path, AssetIn, AssetOut, BulkAssetImportResult, str (+12 more)
...
Cohesion: 0.09
Nodes (21): AdvisorFlow(), PATCH_PILL, AssistantDrawer(), ExplainResponse, Msg, Served, FactCard(), AiStatus (+13 more)
...
Cohesion: 0.08
Nodes (21): BaseScanner, bracket_host(), expand_targets(), main_entrypoint(), parse_ports(), RateLimiter, scanner_base.py — shared foundation for every scanner module.  SCOPE OF THIS MOD, Simple async rate limiter: at most `rate` operations per second. (+13 more)
...
Cohesion: 0.08
Nodes (13): interpret_redis_info(), _probe_redis(), db_scanner.py — fingerprint database services.  WHY: databases are everywhere on, Classify a Redis INFO reply. `unauthenticated_read` is True only when we     act, gate_0_is_passive_profile(), gate_2_host_discovery(), gate_3_port_scan(), gate_5_branch_eligible() (+5 more)
...
Cohesion: 0.10
Nodes (9): Asset, _parse_ts(), PortFact, asset.py — per-host fact model the workflow engine reasons about.  This is an OR, Is liveness unknown, or stale past `threshold`? Threshold is         profile-dep, Is liveness unknown, or stale past `threshold`? Threshold is         profile-dep, Dispatch a real ScanResult into the right sub-structure, keyed         on result, Dispatch a real ScanResult into the right sub-structure, keyed         on result (+1 more)
...
Cohesion: 0.09
Nodes (4): 1fe16c8 stable but some dead code, need to optimize, Product-boundary tests for the single-dashboard Manager API., Initial schema — all tables  Revision ID: 0001 Revises: Create Date: 2026-05-19, Allow the documented 0-1000 finding risk score range.  Revision ID: 0015 Revises
...
Cohesion: 0.12
Nodes (14): AppEnvironmentValidator, CheckResult, ConfigValidator, CookieValidator, CorsValidator, DatabaseURLValidator, DetectionEngineValidator, Vedha Startup Validator ======================= Runs at application boot — befor (+6 more)
...
Cohesion: 0.16
Nodes (17): _env_number(), _error_result(), _facts_from_cache(), LeaseLostError, engine.py — adapt a manager scan job to scanner_module's workflow engine and ret, Execute a scan and return the enriched result bundle.      Args:         scan_ty, Single factory for error result dicts — no copy-paste., Raised when Manager fencing revokes the running attempt. (+9 more)
...
Cohesion: 0.11
Nodes (12): task_runner.py — orchestrates the full lifecycle of a single scan job.  Given a, _fake_run_scan(), Tests for agent/task_runner.py, Return a minimal successful result without doing any real I/O., Verify the submit callback is called with the correct payload., When spool_submit is provided, it's used instead of direct submit., TaskRunner with no-op dependencies (no real scanning)., Verify the submit callback is called with the correct payload. (+4 more)
...
Cohesion: 0.15
Nodes (6): BaseScanner, DBScanner, PortScanner, _build_get(), _extract_sysdescr(), SNMPScanner

### Community 101 - "Community 101"
Cohesion: 0.16
Nodes (13): deadlineTitle(), elapsedPct(), pct(), Sev, SEV_STYLE, SlaItem, SlaRowView(), SlaState (+5 more)
...
Cohesion: 0.23
Nodes (10): HallucinationGuard, Run all relevant checks and return a combined verdict:         ``{valid, issues,, Flag any CVE ID mentioned in ``text`` that isn't in the real finding set., Flag CVSS scores in the text that don't match any real score.          ``actual_, Flag destructive-looking commands that shouldn't appear in a fix guide., LLMUnavailableError, LLMReportGenerator — Claude-backed narrative generation for VAPT reports.  Uses, Raised when the Anthropic SDK or API key is not configured. (+2 more)
...
Cohesion: 0.20
Nodes (7): DatabaseConnectivityValidator, Verifies actual database connectivity at startup., Verifies Redis connectivity at startup., Run all validators. Use in FastAPI lifespan:          from scripts.startup_valid, RedisConnectivityValidator, run_all_validators(), ValidationReport
...
Cohesion: 0.24
Nodes (5): _collect_cves_scores(), _enum(), _finding_scores(), LLMReportGenerator, _uuid()
...
Cohesion: 0.18
Nodes (11): check_hw_bind(), get_hw_id(), HWBindError, Raised when the binary is running on an unauthorized machine., Deterministic per-machine fingerprint built from stable hardware IDs.      Combi, Verify the binary is running on the machine it was compiled for.      Reads HW_B, Raised when one or more fatal checks fail — aborts app startup., StartupAbortError (+3 more)
...
Cohesion: 0.31
Nodes (8): _get_cert_der(), _parse_cert_der(), tls_scanner.py — collect TLS/SSL configuration facts.  METHOD (collection only):, Never send an IP literal as SNI — non-conformant; some servers reject it., Attempt a handshake forcing one protocol version. Returns cipher dict or None., _scan_tls_sync(), _sni(), _try_version()
...
Cohesion: 0.22
Nodes (6): _netbios_session(), parse_smb2_security_mode(), Read signing posture from an SMB2 NEGOTIATE response.      The response carries, _smb1_negotiate(), _smb2_negotiate(), SMBScanner
...
Cohesion: 0.25
Nodes (8): _clamp(), _job_runtime_seconds(), Coerce val to float and clamp to [lo, hi]; fall back to default on junk.     Def, Return the effective whole-job deadline; callers can only reduce it., Translate operator-supplied job params into run_engagement() kwargs.      This i, Coerce val to float and clamp to [lo, hi]; fall back to default on junk.     Def, Translate operator-supplied job params into run_engagement() kwargs.      This i, _tuning_from_params()
...
Cohesion: 0.33
Nodes (4): ADAssessRequest, Neo4jConfig, _run_ad_assessment_and_save(), _set_job_status()
...
Cohesion: 0.40
Nodes (3): HostDiscoveryScanner, Return 'open', 'refused', or None (no response)., Return 'open', 'refused', or None (no response).
...
Cohesion: 0.67
Nodes (2): Validates secrets meet minimum strength requirements., SecretsValidator
...
## Knowledge Gaps
- **1178 isolated node(s):** `Initial schema — all tables  Revision ID: 0001 Revises: Create Date: 2026-05-19`, `Add services and agents tables  Revision ID: 0002 Revises: 0001 Create Date: 202`, `Add enrichment fields index + webhook column to engagements  Revision ID: 0003 R`, `Exploit results, approvals, and audit log tables  Revision ID: 0004 Revises: 000`, `Detection validation: attack_timeline, detection_configs, extend detection_resul` (+1173 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 65`** (1 nodes): `TestResultSpool`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 93`** (1 nodes): `TestServiceIdentifier`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 110`** (2 nodes): `_action()`, `TestDetectionCorrelator`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 128`** (1 nodes): `TestValidateTargetsInScope`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 138`** (1 nodes): `TestScopeGuard`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 144`** (1 nodes): `TestAgentJobCompatibility`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 145`** (1 nodes): `TestPathAnalyzer`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 148`** (2 nodes): `Tests that use the real engine but with no-op callbacks.`, `TestRunnerHeadless`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 152`** (1 nodes): `TestADCSChecker`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 155`** (1 nodes): `TestHallucinationGuard`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 156`** (1 nodes): `TestVersionInRanges`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 157`** (1 nodes): `TestNucleiExploitRunner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 158`** (1 nodes): `TestValidatePayload`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 159`** (1 nodes): `TestExpandTargets`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 160`** (2 nodes): `test_enroll_token_usable_only_while_live_unrevoked_and_under_max_uses()`, `_token()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 161`** (1 nodes): `TestNmapXMLParser`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 172`** (1 nodes): `TestGraphBuilder`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 174`** (1 nodes): `TestParsePorts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 175`** (2 nodes): `Each encryption uses a fresh ephemeral key, so blobs are different.`, `TestEncryptDecryptRoundtrip`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 176`** (1 nodes): `TestSubmitResult`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 186`** (1 nodes): `TestUseCasesResolve`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 187`** (1 nodes): `TestTargetsInExcludes`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 194`** (2 nodes): `ssh_collector.py — credentialed (authenticated) inventory collection for Linux.`, `SSHCollector`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 196`** (1 nodes): `TestKerberoastChecker`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 198`** (1 nodes): `TestGraphVisualizer`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 199`** (1 nodes): `TestMetasploitRPCClient`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 200`** (1 nodes): `TestRequiresApproval`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (1 nodes): `TestTuningFromParams`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (1 nodes): `TestBloodHoundCollector`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 215`** (1 nodes): `TestIngestFile`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 216`** (1 nodes): `TestSigmaRuleGenerator`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 217`** (1 nodes): `TestValidateModule`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 218`** (1 nodes): `TestValidateScope`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 220`** (1 nodes): `TestMergeExclusions`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `TestCvss`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (1 nodes): `TestSIEMParsing`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (2 nodes): `Phase 1: combined scope validation (validate + excludes).`, `TestScopeValidationPipeline`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (2 nodes): `Phase 2: WebSocket message parsing.`, `TestWebSocketMessageProtocol`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `TestFetchEngagementScope`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 251`** (1 nodes): `TestTenantWebSocketSelection`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 252`** (1 nodes): `TestGetAgentJobs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 253`** (1 nodes): `TestCheckHwBind`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 254`** (2 nodes): `Phase 1: result spool with upload retry.`, `TestResultSpoolWithRetry`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 257`** (1 nodes): `TestResolveScanType`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 258`** (1 nodes): `TestTargets`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 261`** (1 nodes): `DiagnosticsReport`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 263`** (1 nodes): `_ConnectSweep`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 264`** (1 nodes): `ServiceBannerScanner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 265`** (1 nodes): `TLSScanner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 266`** (1 nodes): `WebScanner`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 269`** (1 nodes): `Add is_active to users and tenants; add password_expires_at to users.  All exist`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 270`** (1 nodes): `Add fenced execution attempts for agent-dispatched scan jobs.  Revision ID: 0017`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 271`** (1 nodes): `Add Manager-approved device-key probe enrollment and Site policy.  Revision ID:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 279`** (2 nodes): `Validates secrets meet minimum strength requirements.`, `SecretsValidator`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 280`** (2 nodes): `Record a heartbeat from an agent.`, `Record a heartbeat from an agent.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 281`** (1 nodes): `Open the driver and verify connectivity. Returns False on any failure.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 282`** (1 nodes): `Fast port discovery with naabu. Feeds port list to Nmap.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 283`** (1 nodes): `Nmap service enumeration. Accepts port list from Naabu.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 284`** (1 nodes): `Nuclei vulnerability scan — production-ready.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 285`** (1 nodes): `Impacket-based AD enumeration: Kerberoast, AS-REP roast, LDAP anonymous bind.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 286`** (1 nodes): `NetExec SMB validation: signing, null sessions, SMBv1.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 287`** (1 nodes): `testssl.sh TLS/SSL analysis.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 288`** (1 nodes): `Extract HTTP/HTTPS URLs from nmap XML output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 289`** (1 nodes): `EyeWitness screenshot evidence collection.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 290`** (1 nodes): `Safe lateral movement checks — no actual exploitation.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 291`** (1 nodes): `Cloud infrastructure scan (AWS/Azure/GCP).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 292`** (1 nodes): `Fetches credentials from HashiCorp Vault at runtime. Never caches to disk.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 293`** (1 nodes): `Read a KV-v2 secret from Vault.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 294`** (1 nodes): `Verify the Python probe can open what the TypeScript manager sealed (T14 interop`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 295`** (1 nodes): `Reference mock manager for end-to-end probe testing.  Implements the PROBE_PROTO`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 296`** (1 nodes): `Start the HTTPS server in a thread. Returns (httpd, base_url, pin_b64).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 297`** (1 nodes): `End-to-end probe test: real probe process ↔ reference mock manager over HTTPS.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 298`** (1 nodes): `Deterministic stand-ins emitting realistic output for 127.0.0.1.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 299`** (1 nodes): `ThreadingHTTPServer`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `FindingSeverity` connect `Community 55` to `Community 32`, `Community 56`, `Community 76`, `Community 1`, `Community 46`, `Community 66`, `Community 104`, `Community 18`, `Community 182`, `Community 3`, `Community 212`, `Community 4`, `Community 142`, `Community 152`, `Community 214`, `Community 196`, `Community 63`, `Community 90`, `Community 127`, `Community 199`, `Community 157`, `Community 200`, `Community 217`, `Community 158`, `Community 218`, `Community 0`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Why does `Transport` connect `Community 164` to `Community 15`, `Community 106`, `Community 131`, `Community 223`, `Community 250`, `Community 180`, `Community 245`, `Community 248`, `Community 244`, `Community 246`, `Community 247`, `Community 249`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `Finding` connect `Community 3` to `Community 0`, `Community 22`, `Community 95`, `Community 18`, `Community 182`, `Community 10`, `Community 102`, `Community 14`, `Community 55`, `Community 4`, `Community 212`, `Community 184`?**
...
- **Should `Community 0` be split into smaller, more focused modules?**
  _Cohesion score 0.018974099992785515 - nodes in this community are weakly interconnected._
- **Should `Community 1` be split into smaller, more focused modules?**
  _Cohesion score 0.020878601794992915 - nodes in this community are weakly interconnected._