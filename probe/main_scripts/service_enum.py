...
from .scanner_base import (
    ...
    setup_logging, main_entrypoint, bracket_host,
    is_root, icmp_ping, ttl_os_guess, LOG,
)
...
def guess_os(open_ports: set, services: dict, hostnames: dict,
    ...
    # TTL family signal (only present when root ran an ICMP probe).
    ttl_fam = ttl_os_guess(ttl).get("os_family") if ttl else None
    if ttl_fam:
...
class ServiceEnumScanner(BaseScanner):
    ...
    async def scan_target(self, target: str) -> list[ScanResult]:
        ...
        # 2b) TTL fingerprint — privileged (root) ICMP probe, best-effort.
        ttl = None
        if is_root():
            icmp = await asyncio.to_thread(icmp_ping, target, min(self.timeout, 2.0))
            if icmp:
                ttl = icmp.get("ttl")
